...
const p = Object.keys(cov).find(k => k.endsWith('command-registry.ts'));
if (!p) {
  console.log('command-registry.ts not found in coverage');
  process.exit(1);
}
const d = cov[p];
const b = d.b || {};
...
console.log(`command-registry.ts: ${covered}/${total} branches covered (${((covered/total)*100).toFixed(2)}%)`);
