...
const p = Object.keys(cov).find(k => k.endsWith('team-manager.ts'));
if (!p) {
  console.log('team-manager.ts not found in coverage');
  process.exit(1);
}
const d = cov[p];
const b = d.b || {};
...
console.log(`team-manager.ts: ${covered}/${total} branches covered (${((covered/total)*100).toFixed(2)}%)`);
