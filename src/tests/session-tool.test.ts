...
import { resetSessionTool, createSessionTool } from '../tools/session-tool.js';
import { createMockRuntime, createMockSession } from './utils.js';

describe('SessionTool', () => {
    let runtime: any;
    let tool: ReturnType<typeof createSessionTool>;
    ...
    describe('create operation', () => {
        it('should create a new child session', async () => {
            const result = await tool.execute('create1', { operation: 'create', name: 'my-child' });
        ...
        it('should auto-generate name if not provided', async () => {
            const result = await tool.execute('create2', { operation: 'create' });
            expect(result.details.name).toMatch(/^child-\d+$/);
        });

        it('should add custom tags', async () => {
            const result = await tool.execute('create3', {
                operation: 'create',
                tags: ['debug', 'test'],
            });
        ...
        it('should set parent to current active session', async () => {
            ...
            const infoResult = await tool.execute('info_child2', { operation: 'info', sessionId: child2Id });
    ...
    describe('switch operation', () => {
        it('should switch to a specific session by ID', async () => {
            const createResult = await tool.execute('switch_create', { operation: 'create', name: 'child' });
        ...
        it('should switch to parent using alias', async () => {
            const createResult = await tool.execute('switch_parent_', { operation: 'create' });
            await tool.execute('switch_parent_alias', { operation: 'switch', sessionId: 'parent' });

            const status = await tool.execute('status_parent', { operation: 'status' });
            expect(status.content[0].text).toContain('parent');
        });

        it('should throw when switching to non-existent session', async () => {
            const result = await tool.execute('switch_fail', { operation: 'switch', sessionId: 'nonexistent' });
            expect(result.isError).toBe(true);
            expect(result.content[0].text).toContain('Error: Session not found');
        });

        it('should throw when already active', async () => {
            const createResult = await tool.execute('switch_already', { operation: 'create' });
            const childId = createResult.details.sessionId;
            const result = await tool.execute('switch_already2', { operation: 'switch', sessionId: childId });
            expect(result.isError).toBe(true);
    ...
    describe('list operation', () => {
        it('should list all sessions by default', async () => {
            ...
            const result = await tool.execute('list3', { operation: 'list' });
        ...
        it('should filter by state', async () => {
            ...
            const resultActive = await tool.execute('list_f3', { operation: 'list', filterState: 'active' });
            ...
            const resultInactive = await tool.execute('list_f4', { operation: 'list', filterState: 'inactive' });
        ...
        it('should limit results', async () => {
            ...
            const result = await tool.execute('list_limited', { operation: 'list', limit: 3 });
        ...
        it('should sort by name', async () => {
            ...
            const result = await tool.execute('sort3', { operation: 'list', sortBy: 'name' });
            const lines = result.content[0].text.split('\n');
            // Find lines that contain children (excluding the first "Sessions (...)" line)
            const childLines = lines.filter(l => l.includes('child-'));
    ...
    describe('info operation', () => {
        it('should show detailed info for a session', async () => {
            const createResult = await tool.execute('info1', { operation: 'create', name: 'info-child', tags: ['tag1', 'tag2'] });
            const childId = createResult.details.sessionId;

            const result = await tool.execute('info2', { operation: 'info', sessionId: childId });
        ...
        it('should default to active session if no sessionId provided', async () => {
            const result = await tool.execute('info_active', { operation: 'info' });
    ...
    describe('rename operation', () => {
        it('should rename a session', async () => {
            const createResult = await tool.execute('rename1', { operation: 'create', name: 'old-name' });
            const sessionId = createResult.details.sessionId;

            const result = await tool.execute('rename2', {
                operation: 'rename',
                sessionId,
                name: 'new-name',
            });
        ...
        it('should require name parameter', async () => {
            const createResult = await tool.execute('rename_err1', { operation: 'create' });
            const sessionId = createResult.details.sessionId;

            const result = await tool.execute('rename_err2', {
                operation: 'rename',
                sessionId,
            });
            expect(result.isError).toBe(true);
    ...
    describe('tag operation', () => {
        it('should add tags', async () => {
            const createResult = await tool.execute('tag_add1', { operation: 'create' });
            const sessionId = createResult.details.sessionId;

            const result = await tool.execute('tag_add2', {
                operation: 'tag',
                sessionId,
                tagAction: 'add',
                tags: ['x', 'y'],
        ...
        it('should remove tags', async () => {
            const createResult = await tool.execute('tag_rem1', { operation: 'create', tags: ['a', 'b'] });
            const sessionId = createResult.details.sessionId;

            const result = await tool.execute('tag_rem2', {
                operation: 'tag',
                sessionId,
                tagAction: 'remove',
                tags: ['a'],
    ...
    describe('delete operation', () => {
        it('should delete a session from registry', async () => {
            const createResult = await tool.execute('del1', { operation: 'create' });
            const sessionId = createResult.details.sessionId;

            const result = await tool.execute('del2', { operation: 'delete', sessionId });

            expect(result.content[0].text).toContain('Deleted session from registry');

            // Verify it's gone from list
            const listResult = await tool.execute('del3', { operation: 'list', includeDisposed: true });
        ...
        it('should delete active session if no sessionId provided', async () => {
            ...
            const result = await tool.execute('del_active2', { operation: 'delete' });
            expect(result.content[0].text).toContain('Deleted session');

            // Active should now be parent
            const status = await tool.execute('del_active3', { operation: 'status' });
            expect(status.content[0].text).toContain('parent');
        });
    });
    ...
    describe('history operation', () => {
        it('should show operation history', async () => {
            ...
            const result = await tool.execute('hist3', { operation: 'history', limit: 5 });
        ...
        it('should respect limit parameter', async () => {
            ...
            const result = await tool.execute('histlim', { operation: 'history', limit: 3 });
    ...
    describe('status operation', () => {
        it('should show current status', async () => {
            const result = await tool.execute('stat1', { operation: 'status' });
        ...
        it('should reflect active session changes', async () => {
            const createResult = await tool.execute('stat2', { operation: 'create', name: 'child' });
            const childId = createResult.details.sessionId;
            await tool.execute('stat3', { operation: 'switch', sessionId: childId });

            const status = await tool.execute('stat4', { operation: 'status' });
            expect(status.content[0].text).toContain(childId);
        });
    });
    ...
    describe('diagnostics operation', () => {
        it('should show internal diagnostics', async () => {
            ...
            const result = await tool.execute('diag2', { operation: 'diagnostics' });
    ...
    describe('error handling', () => {
        it('should return error for unknown operation', async () => {
            const result = await tool.execute('err1', { operation: 'unknown' } as any);
            expect(result.isError).toBe(true);
            expect(result.content[0].text).toContain('Unknown operation');
        });

        it('should validate required parameters', async () => {
            const result = await tool.execute('err2', { operation: 'rename' } as any);
            expect(result.isError).toBe(true);
            expect(result.content[0].text).toContain('Name is required');
        });
        ...
        it('should catch runtime errors and return isError', async () => {
            ...
            const result = await tool.execute('err3', { operation: 'switch', sessionId: 'invalid' });
            expect(result.isError).toBe(true);
    ...
    describe('Integration scenarios', () => {
        ...
        it('should support tagging and renaming', async () => {
            const create = await tool.execute('tagint1', { operation: 'create', name: 'debugging' });
