...
import { SessionRegistry, SessionMetadata, SessionState } from "./session-registry.js";
...
export class MultiSessionManager {
    ...
    async switchToLastChild(): Promise<void> {
        ...
        const mostRecent = children.sort(
            (a, b) => b.createdAt.getTime() - a.createdAt.getTime()
        )[0];
    ...
    async dispose(sessionId: string | null = null, disposeRuntime: boolean = false): Promise<void> {
        ...
        if (meta.id === this.rootSessionId && !disposeRuntime) {
            throw new Error("Cannot dispose parent session without disposing runtime. Use disposeRuntime=true if intended.");
        }
     ...
     */
    getHistory(limit?: number): import("./session-registry.js").SessionHistoryEntry[] {
