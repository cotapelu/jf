...
function createTodoTool(api: ExtensionAPI): ToolDefinition<any, TodoToolDetails> {
  ...
  return {
    ...
    promptSnippet: "plugin.todos({ OPERATION: {...} }). All params are OBJECTS (not JSON strings). Ops: add_phase, add_task, update, remove_task, delete, list.",
    promptGuidelines: [
      "RULE: Operation is the KEY. plugin.todos({ OPERATION: params }), NOT { operation: name, params: {...} }",
      "",
      "TEMPLATES:",
      "• add_phase: plugin.todos({ add_phase: { name: 'Phase', tasks: [{ content: 'Task' }] } })",
      "• add_task:  plugin.todos({ add_task: { phase: 'phase-1', content: 'Task' } })",
      "  ⚠️ phase must be ID (phase-1), NOT name",
      "• update:    plugin.todos({ update: { id: 'task-5', status: 'in_progress' } })",
      "  Batch:    plugin.todos({ update: { ids: ['task-1','task-2'], status: 'completed' } })",
      "  status: pending|in_progress|completed|abandoned",
      "• remove_task: plugin.todos({ remove_task: { id: 'task-3' } })",
      "• delete: plugin.todos({ delete: {} })",
      "• list: plugin.todos({ list: {} })",
      "",
      "AUTO-RULES:",
      ...
      "• Task IDs are auto: task-1, task-2... Find them by running plugin.todos({ list: {} })",
    ],
    parameters: {},
