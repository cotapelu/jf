...
function findFunctionDeclarations(node, targetName, results = []) {
  ...
  for (const key in node) {
    if (Object.hasOwn(node, key)) {
      ...
      if (Array.isArray(child)) {
        child.forEach(item => { if (item && typeof item === 'object') findFunctionDeclarations(item, targetName, results); });
      } else if (child && typeof child === 'object') {
        findFunctionDeclarations(child, targetName, results);
      }
    }
  }
  return results;
...
const funcNodes = findFunctionDeclarations(ast.ast, 'createTodoTool');
if (funcNodes.length === 0) { console.error('createTodoTool not found'); process.exit(1); }
if (funcNodes.length > 1) { console.error('Multiple createTodoTool found'); process.exit(1); }
...
`;
const newContent = code.substring(0, start) + newCode + code.substring(end);
fs.writeFileSync(path, newContent, 'utf8');
console.log('Refactored createTodoTool using AST');
