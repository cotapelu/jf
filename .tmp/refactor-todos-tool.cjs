...
const startSig = 'function createTodoTool(api: ExtensionAPI): ToolDefinition<any, TodoToolDetails> {';
const startIdx = code.indexOf(startSig);
if (startIdx === -1) { console.error('createTodoTool not found'); process.exit(1); }

// Scan for matching closing brace, respecting strings and comments.
let braceCount = 0;
let inSingleString = false, inDoubleString = false, inTemplate = false;
let inSingleLineComment = false, inMultiLineComment = false;
let escaped = false;
let i = startIdx;
for (; i < code.length; i++) {
  const char = code[i];
  ...
  if (inSingleString) {
    ...
    if (char === '\'') { inSingleString = false; }
    continue;
  }
  if (inDoubleString) {
    if (char === '\\') { escaped = true; continue; }
    if (char === '"') { inDoubleString = false; }
    continue;
  }
  if (inTemplate) {
    if (char === '`') { inTemplate = false; continue; }
    if (char === '$' && nextChar === '{') {
      // Expression start: treat normally, the following '{' will be counted
    }
    continue;
  }
  ...
  if (char === '\'') { inSingleString = true; continue; }
  if (char === '"') { inDoubleString = true; continue; }
  ...
  if (braceCount === 0 && i > startIdx) {
    // We've closed the initial brace that opened the function
    break;
  }
}
if (braceCount !== 0) {
  console.error('Failed to find matching closing brace');
  process.exit(1);
}
const endIdx = i + 1; // include the closing '}'
...
`;
const newContent = code.substring(0, startIdx) + newCode + code.substring(endIdx);
fs.writeFileSync(path, newContent, 'utf8');
console.log('Refactored todos-tool.ts');
