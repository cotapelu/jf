 ...
 */
function shouldDropFile(filePath, opts) {
 ...
 */
export async function compactFileContent(content, filePath, opts = {}) {
 ...
 */
export async function contextCompactDirectory(dirPath, opts = {}) {
    const defaults = {
        tokenLimit: 128000,
        dropTests: true,
        dropDocs: true,
...
export async function contextCompactMessages(messages, opts = {}) {
    ...
    function scoreMessage(msg, idx, total, _options) {
        ...
        // Questions
        if (msg.role === "user" && /\?$/.test(content.trim()))
            contentScore += 25;
        // Code blocks
        const codeBlocks = (content.match(/```/g) || []).length;
        contentScore += codeBlocks * 15;
        // File paths
        if (/\.(ts|js|py|java|cpp|h|json|yaml|yml|md|txt|csv)\b/.test(content))
            contentScore += 10;
        // Commands in assistant
        if (msg.role === "assistant" && content.includes("$ ") && content.length > 2000)
            contentScore += 20;
    ...
    // LLM summarization
    async function summarizeWithLLM(content, targetTokens, options) {
        if (!options.apiKey)
            return null;
        ...
        try {
            ...
            if (options.verbose)
                console.warn("LLM summarize failed:", e);
            return null;
        }
    }
    // Check Q-A pair
    ...
    for (let i = 0; i < cloned.length; i++) {
        ...
        if (options.trimWhitespace)
            content = trimWhitespace(content);
        if (options.removeComments && content.includes("//"))
            content = stripCodeComments(content);
        ...
        if (tokensNow > 5000 && options.useLLM && options.apiKey) {
            ...
            if (summarized && estimate(summarized) < tokensNow) {
                content = summarized;
                if (options.verbose)
                    actions.push(`LLM-summarized message ${i} (${tokensNow}→${estimate(content)} tokens)`);
            }
        }
    ...
    if (keptTokens > options.tokenLimit) {
        ...
        for (const idx of sortedByScore) {
            ...
            // Don't break pairs
            let canDrop = true;
            if (idx > 0 && keepIndices.has(idx - 1) && isPair(cloned[idx - 1], cloned[idx]))
                canDrop = false;
...
//# sourceMappingURL=context-compactor-tool.js.map