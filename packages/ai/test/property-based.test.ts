...
describe("property-based tests", () => {
	...
	it("should handle arbitrary context sizes", async () => {
		...
		await fc.assert(
			fc.asyncProperty(fc.array(fc.string(), { minLength: 0, maxLength: 20 }), async (messages) => {
				provider.setResponses([fauxAssistantMessage("Response to arbitrary context")]);
	...
	it("should preserve message ordering across transformations", async () => {
		...
		await fc.assert(
			fc.asyncProperty(fc.array(fc.string(), { minLength: 1, maxLength: 10 }), async (texts) => {
				// Echo the last message to verify ordering
				const lastText = texts[texts.length - 1];
				provider.setResponses([fauxAssistantMessage(`Echo: ${lastText}`)]);
		...
		await fc.assert(
			fc.asyncProperty(
				...
				async (hasReasoning, thoughts) => {
					...
					provider.setResponses([fauxAssistantMessage([...thinkingContent, ...textContent])]);
	...
	it("should correctly serialize/deserialize thinking blocks", async () => {
		...
		await fc.assert(
			fc.asyncProperty(fc.array(fc.string(), { minLength: 1, maxLength: 10 }), async (thinkingTexts) => {
				const provider = registerFauxProvider({
					models: [{ id: "test", name: "Test", reasoning: true }],
				});

				const thinkingBlocks = thinkingTexts.map((t) => fauxThinking(t));
				provider.setResponses([fauxAssistantMessage([...thinkingBlocks, fauxText("Done")])]);
	...
	it("should handle various thinking levels", async () => {
		...
		await fc.assert(
			fc.asyncProperty(fc.oneof(...levels.map(fc.constant)), async (level) => {
				const provider = registerFauxProvider({
					models: [
						{
							id: "test",
				...
				provider.setResponses([
					level === "off"
						...
						: fauxAssistantMessage([fauxThinking(`Thinking at level: ${level}`), fauxText("Done")]),
				]);
	...
	it("should generate valid model IDs", async () => {
		...
		await fc.assert(
			fc.asyncProperty(fc.string({ minLength: 1, maxLength: 50 }), fc.boolean(), async (name, hasReasoning) => {
				const provider = registerFauxProvider({
					models: [
						{
	...
	it("should handle concurrent stream operations", async () => {
		...
		await fc.assert(
			fc.asyncProperty(fc.integer({ min: 2, max: 5 }), async (concurrentCount) => {
				const providers = Array.from({ length: concurrentCount }, (_, i) => {
					const provider = registerFauxProvider({
						models: [{ id: `model-${i}`, name: `Model ${i}` }],
					});
					provider.setResponses([fauxAssistantMessage(`Response from model ${i}`)]);
					return provider;
				});
	...
	it("should produce valid stream events", async () => {
		...
		await fc.assert(
			fc.asyncProperty(fc.array(fc.string(), { minLength: 1, maxLength: 5 }), async (texts) => {
				...
				provider.setResponses([fauxAssistantMessage(texts.map((t) => fauxText(t)))]);
