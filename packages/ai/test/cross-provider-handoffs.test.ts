...
describe("cross-provider handoffs", () => {
    ...
    openai.provider.setResponses([
      {
        ...
        provider: "openai",
        model: "gpt-4o",
        usage: { input: 100, output: 150, cacheRead: 0, cacheWrite: 0, totalTokens: 250, cost: { input: 0.001, output: 0.002, cacheRead: 0, cacheWrite: 0, total: 0.003 } },
    ...
    anthropic.provider.setResponses([
      {
        ...
        usage: { input: 120, output: 130, cacheRead: 0, cacheWrite: 0, totalTokens: 250, cost: { input: 0.0015, output: 0.0025, cacheRead: 0, cacheWrite: 0, total: 0.004 } },
        stopReason: "stop" as const,
        timestamp: Date.now(),
      } as AssistantMessage
    ]);
  ...
  it("should preserve thinking blocks across provider handoffs", async () => {
    ...
    google.provider.setResponses([
      {
        ...
        usage: { input: 80, output: 120, cacheRead: 0, cacheWrite: 0, totalTokens: 200, cost: { input: 0.0005, output: 0.001, cacheRead: 0, cacheWrite: 0, total: 0.0015 } },
        stopReason: "stop" as const,
        timestamp: Date.now(),
      } as AssistantMessage
    ]);
    ...
    openai.provider.setResponses([
      {
        ...
        provider: "openai",
        model: "gpt-4o",
        usage: { input: 150, output: 100, cacheRead: 0, cacheWrite: 0, totalTokens: 250, cost: { input: 0.001, output: 0.0015, cacheRead: 0, cacheWrite: 0, total: 0.0025 } },
        stopReason: "stop" as const,
        timestamp: Date.now(),
      } as AssistantMessage
    ]);
    ...
    const thinkingBlocks1 = response1.content.filter(
    ...
    const thinkingBlocks2 = response2.content.filter(
      (c): c is ThinkingContent => c.type === "thinking"
    );
    ...
    google.provider.setResponses([
      {
        ...
        provider: "google",
        model: "gemini-1.5-pro",
        usage: { input: 200, output: 80, cacheRead: 0, cacheWrite: 0, totalTokens: 280, cost: { input: 0.0005, output: 0.001, cacheRead: 0, cacheWrite: 0, total: 0.0015 } },
        stopReason: "stop" as const,
        timestamp: Date.now(),
      } as AssistantMessage
    ]);
      ...
      p.provider.setResponses([
        {
          ...
          api: p.modelDef.id.includes("openai") ? "openai" : 
               p.modelDef.id.includes("anthropic") ? "anthropic" : "google",
          provider: p.modelDef.id.includes("openai") ? "openai" : 
                    p.modelDef.id.includes("anthropic") ? "anthropic" : "google",
          model: p.modelDef.id,
          usage: { input: 50, output: 50, cacheRead: 0, cacheWrite: 0, totalTokens: 100, cost: { input: 0.0005, output: 0.0005, cacheRead: 0, cacheWrite: 0, total: 0.001 } },
          stopReason: "stop" as const,
          timestamp: Date.now(),
        } as AssistantMessage
      ]);
    ...
    mistral.provider.setResponses([
      {
        ...
        provider: "mistral",
        model: "mistral-large",
        usage: { input: 60, output: 40, cacheRead: 0, cacheWrite: 0, totalTokens: 100, cost: { input: 0.0004, output: 0.0006, cacheRead: 0, cacheWrite: 0, total: 0.001 } },
        stopReason: "stop" as const,
        timestamp: Date.now(),
      } as AssistantMessage
    ]);
      ...
      p.provider.setResponses([
        {
          ...
          api: p.modelDef.id.includes("openai") ? "openai" : 
               p.modelDef.id.includes("google") ? "google" : "anthropic",
          provider: p.modelDef.id.includes("openai") ? "openai" : 
                    p.modelDef.id.includes("google") ? "google" : "anthropic",
          model: p.modelDef.id,
          usage: { input: 50, output: 50, cacheRead: 0, cacheWrite: 0, totalTokens: 100, cost: { input: 0.0005, output: 0.0005, cacheRead: 0, cacheWrite: 0, total: 0.001 } },
        ...
        } as AssistantMessage,
        {
          ...
          provider: p.modelDef.id.includes("openai") ? "openai" : 
                    p.modelDef.id.includes("google") ? "google" : "anthropic",
          model: p.modelDef.id,
          usage: { input: 30, output: 40, cacheRead: 0, cacheWrite: 0, totalTokens: 70, cost: { input: 0.0003, output: 0.0004, cacheRead: 0, cacheWrite: 0, total: 0.0007 } },
          stopReason: "stop" as const,
          timestamp: Date.now(),
        } as AssistantMessage
      ]);
