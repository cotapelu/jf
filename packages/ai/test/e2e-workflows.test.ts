...
describe("end-to-end workflows", () => {
  ...
  beforeEach(() => {
    provider = registerFauxProvider({
      models: [
        { id: "e2e-test", name: "E2E Test Model" }
      ]
    });
  ...
  it("should handle multi-turn conversation", async () => {
    ...
    const context2: Context = {
      messages: [
        ...
        { role: "assistant", content: [{ type: "text", text: "I can help you calculate that." }], timestamp: Date.now() },
  ...
  it("should execute tool calls and process results", async () => {
    ...
    provider.setResponses([
      ...
      fauxAssistantMessage([
        fauxText("I found the information you requested.")
      ])
    ]);
  ...
  it("should handle error conditions in tool workflows", async () => {
    ...
    provider.setResponses([
      fauxAssistantMessage([
        fauxText("Attempting to process..."),
        fauxToolCall("process_data", { data: "test" })
      ]),
      fauxAssistantMessage([
      ...
      fauxAssistantMessage([
        fauxText("Success using alternative method!")
      ])
    ]);
  ...
  it("should handle thinking and reasoning workflows", async () => {
    ...
    const thinkingBlocks = response.content.filter(
      (c: any): c is { type: "thinking"; thinking: string } => 
        c.type === "thinking"
    );

    expect(thinkingBlocks.length).toBe(3);
    expect(response.content[response.content.length - 1].text)
      .toContain("solution");
  });
  ...
  it("should handle long context workflows", async () => {
    ...
    provider.setResponses([
      fauxAssistantMessage("I understand the full context.")
    ]);
  ...
  it("should stream complete conversation events", async () => {
    ...
    provider.setResponses([
      fauxAssistantMessage("Streaming response")
    ]);
  ...
  it("should maintain context across multiple operations", async () => {
    ...
    for (let i = 0; i < operations; i++) {
      ...
      p.setResponses([
        fauxAssistantMessage(`Operation ${i + 1} complete`)
      ]);
  ...
  it("should handle context pruning scenarios", async () => {
    ...
    provider.setResponses([
      fauxAssistantMessage("I can work with this context")
    ]);
  ...
  it("should continue after error in multi-step workflow", async () => {
    ...
    providers[1].setResponses([
      fauxAssistantMessage("Recovered successfully")
    ]);
    ...
    providers.forEach(p => p.unregister());
  });
});