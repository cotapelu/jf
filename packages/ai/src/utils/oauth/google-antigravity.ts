...
const getClientId = () => {
	const env = process.env.GOOGLE_ANTIGRAVITY_CLIENT_ID;
	if (!env) throw new Error("GOOGLE_ANTIGRAVITY_CLIENT_ID environment variable is required");
	return env;
};

const getClientSecret = () => {
	const env = process.env.GOOGLE_ANTIGRAVITY_CLIENT_SECRET;
	if (!env) throw new Error("GOOGLE_ANTIGRAVITY_CLIENT_SECRET environment variable is required");
	return env;
};
