...
export class LLMExtension {
	 ...
	 */
	async queryKnowledge(question: string, options: Partial<LLMWikiConfig> = {}): Promise<WikiResult[]> {
		...
		try {
			...
			const results = parsed.results.slice(0, effectiveConfig.maxResults).map((result) => ({
				...result,
